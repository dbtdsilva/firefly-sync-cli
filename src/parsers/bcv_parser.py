...
class BcvParser(Parser):
    ...
    def parse(file: str) -> List[ParsedTransaction]:
        data = Parser.read_table_from_excel(file=file,
                                            start_text='Execution date')
        # Now data contains the parsed CSV data
        transactions = []
        for row in data:
            transactions.append(BcvParser.__parse_row(file, row))
        return transactions

    def __parse_row(file, row) -> ParsedTransaction:
        ...
        if (debit is not None and credit is not None) or (debit is None and credit is None):
